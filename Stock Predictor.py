...
forecast[["ds", "yhat"]].to_csv(f"results/{ticker}_historical_and_forecast.csv", index=False)
...
forecast[forecast['ds'] >= today][["ds", "yhat"]].to_csv(f"results/{ticker}_forecast.csv", index=False)
...
plt.savefig(f"results/{ticker}_components.png")
plt.show()
